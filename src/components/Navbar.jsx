...
import { fetchNotifications, addToast, toggleTheme } from "../store/uiSlice";
...
export const NavBar = () => {
  ...
  const handleLogout = () => {
    dispatch(logoutUser());
    dispatch(
      addToast({ message: "Başarıyla çıkış yapıldı.", type: "success" }),
    );
    navigate("/");
  };
    ...
    <header className="bg-surface-container shadow-sm border-b border-outline-variant sticky top-0 z-50">
      <nav className="flex justify-between items-center px-margin-mobile md:px-margin-desktop py-md w-full max-w-7xl mx-auto">
        <div className="flex items-center gap-xl">
          <Link
            to="/"
            className="font-headline-h3 text-headline-h3 text-ember-orange font-bold"
          >
            Kütüphane++
          </Link>
          <div className="hidden md:flex items-center gap-lg">
            <Link
              className={`font-label-sm text-label-sm ${location.pathname === "/" ? "text-ember-orange font-bold border-b-2 border-ember-orange pb-1" : "text-on-surface-variant font-medium hover:text-ember-orange"}`}
              to="/"
            >
              Ana Sayfa
            </Link>
            <Link
              className={`font-label-sm text-label-sm ${location.pathname === "/search" || location.pathname === "/books" ? "text-ember-orange font-bold border-b-2 border-ember-orange pb-1" : "text-on-surface-variant font-medium hover:text-ember-orange"}`}
              to="/search"
            >
              Kitap Ara
            </Link>
            <Link
              className={`font-label-sm text-label-sm ${location.pathname === "/digital-library" ? "text-ember-orange font-bold border-b-2 border-ember-orange pb-1" : "text-on-surface-variant font-medium hover:text-ember-orange"}`}
              to="/digital-library"
            >
              Dijital Kütüphane
            </Link>
            <Link
              className={`font-label-sm text-label-sm ${location.pathname === "/blog" ? "text-ember-orange font-bold border-b-2 border-ember-orange pb-1" : "text-on-surface-variant font-medium hover:text-ember-orange"}`}
              to="/blog"
            >
              Blog
            </Link>
            <Link
              className="text-on-surface-variant font-medium hover:text-ember-orange transition-colors duration-200 font-label-sm text-label-sm"
              to="/events"
            >
              Etkinlikler
            </Link>
        ...
        <div className="flex items-center gap-md">
          <div className="flex items-center gap-sm">
            <button
              ...
              className="p-2 rounded-full hover:bg-surface-container-highest transition-all scale-95 active:opacity-80"
            >
