...
const ProfilePage = () => {
  // Okan'ın var olan auth veya user slice'ından veriyi güvenli çekiyoruz
  const authUser = useSelector((state) => state.auth?.user) || {};

  const [user, setUser] = useState({
    name: authUser.name || 'Sudenaz Sangür',
    email: authUser.email || 'sudenaz@nisantasi.edu.tr',
    department: authUser.department || 'Management Information Systems',
  });
  ...
  const handlePasswordSubmit = (e) => {
    ...
    setPasswordStatus({ type: 'success', msg: 'Şifreniz başarıyla güncellendi.' });
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold text-slate-800 mb-6">Profil & Güvenlik Ayarları</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Profil Bilgileri Formu */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <h2 className="text-lg font-semibold text-slate-800 mb-4">Kişisel Profil Bilgileri</h2>
          <form onSubmit={handleProfileSubmit} className="space-y-4">
            <div>
              <label className="text-sm font-medium text-slate-600 block mb-1">Ad Soyad</label>
              <input type="text" className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-none" value={user.name} onChange={(e) => setUser({...user, name: e.target.value})} />
            </div>
            <div>
              <label className="text-sm font-medium text-slate-600 block mb-1">E-posta Adresi</label>
            ...
            <div>
              <label className="text-sm font-medium text-slate-600 block mb-1">Akademik Bölüm</label>
            ...
            {profileSuccess && <div className="p-3 bg-emerald-50 text-emerald-700 text-sm rounded-xl">Değişiklikler başarıyla kaydedildi.</div>}
            <button type="submit" className="w-full bg-indigo-600 text-white font-medium py-2 rounded-xl hover:bg-indigo-700 transition-colors">Bilgileri Güncelle</button>
          </form>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-slate-800 text-white font-medium py-2 rounded-xl hover:bg-slate-900 transition-colors">Şifreyi Değiştir</button>
          </form>
        </div>
      </div>
    </div>
