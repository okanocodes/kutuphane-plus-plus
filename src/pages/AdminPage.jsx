...
export const AdminPage = () => {
  ...
  const { books, authors, categories, publishers } = useSelector(
    (state) => state.books,
  );
  const { users, borrowedBooks, penalties } = useSelector(
    (state) => state.users,
  );
  const { reservations, studyDesks, meetingRooms, events } = useSelector(
  ...
  useEffect(() => {
    document.title = "Kütüphane++ — Yönetim Paneli";
  }, []);
  ...
  const getAuthorName = (authorId) =>
    authors.find((a) => String(a.id) === String(authorId))?.name ||
    `Yazar #${authorId}`;
  const getCategoryName = (categoryId) =>
    categories.find((c) => String(c.id) === String(categoryId))?.name ||
    `Kategori #${categoryId}`;
  const getPublisherName = (publisherId) =>
    publishers.find((p) => String(p.id) === String(publisherId))?.name ||
    `Yayınevi #${publisherId}`;
  ...
  const deskOccupancy = Math.round(
    (studyDesks.filter((d) => d.status !== "available").length /
      (studyDesks.length || 1)) *
      100,
  );
  const roomOccupancy = Math.round(
    (meetingRooms.filter((r) => r.status !== "available").length /
  ...
  const handleAddCategory = async (name) => {
    ...
    if (addCategory.fulfilled.match(action)) {
      dispatch(
        addToast({ message: "Yeni kategori eklendi.", type: "success" }),
      );
      return action.payload;
    }
  };
  ...
  const handleAddPublisher = async (name) => {
    ...
    if (addPublisher.fulfilled.match(action)) {
      dispatch(
        addToast({ message: "Yeni yayınevi eklendi.", type: "success" }),
      );
      return action.payload;
    }
  };
  ...
  const handleGenerateISBN = () => {
    ...
    dispatch(
      addToast({ message: "ISBN-13 başarıyla üretildi.", type: "info" }),
    );
  };
  ...
  const handleBookSubmit = async (e) => {
    ...
    if (editingItem) {
      const resultAction = await dispatch(
        updateBook({ id: editingItem.id, bookData: data }),
      );
      if (updateBook.fulfilled.match(resultAction)) {
        ...
        dispatch(
          addToast({
            message:
              resultAction.payload || "Kitap güncellenirken hata oluştu.",
            type: "error",
          }),
        );
      }
      ...
      if (addBook.fulfilled.match(resultAction)) {
        ...
        dispatch(
          addToast({
            message: resultAction.payload || "Kitap eklenirken hata oluştu.",
            type: "error",
          }),
        );
      }
  ...
  const handleUserSubmit = async (e) => {
    ...
    if (editingItem) {
      const resultAction = await dispatch(
        updateUser({ id: editingItem.id, userData: userForm }),
      );
      if (updateUser.fulfilled.match(resultAction)) {
        dispatch(
          addToast({ message: "Kullanıcı güncellendi.", type: "success" }),
        );
      }
    } else {
  ...
  const handleSpaceSubmit = async (e) => {
    ...
    dispatch(
      addToast({
        message: "Çalışma alanı başarıyla tanımlandı.",
        type: "success",
      }),
    );
  ...
  const handleEventSubmit = async (e) => {
    ...
    if (editingItem) {
      ...
      dispatch(
        addToast({ message: "Yeni etkinlik oluşturuldu.", type: "success" }),
      );
    }
  ...
  const handleStatusChange = async (id, newStatus, bookId) => {
    const resultAction = await dispatch(
      updateReservationStatus({ id, status: newStatus, bookId }),
    );
    if (updateReservationStatus.fulfilled.match(resultAction)) {
      dispatch(
        addToast({
          message: `Rezervasyon durumu '${newStatus}' olarak güncellendi.`,
          type: "success",
        }),
      );
      dispatch(fetchReservations());
  ...
  const handleReturn = async (recordId, bookId) => {
    const resultAction = await dispatch(
      returnBook({ borrowedRecordId: recordId, bookId }),
    );
    if (returnBook.fulfilled.match(resultAction)) {
      dispatch(
        addToast({ message: "Kitap başarıyla iade alındı.", type: "success" }),
      );
      dispatch(fetchBorrowedBooks());
      dispatch(fetchBooks());
    } else {
  ...
  const handleIssueLoan = async (e) => {
    ...
    if (!loanForm.userId || !loanForm.bookId) {
      dispatch(
        addToast({
          message: "Lütfen kullanıcı ve kitap seçin.",
          type: "warning",
        }),
      );
      return;
    ...
    const result = await dispatch(
      borrowBook({ userId: loanForm.userId, bookId: loanForm.bookId }),
    );
  ...
  const handleSettingsSave = async (e) => {
    ...
    dispatch(
      addToast({
        message: "Sistem limit ayarları kaydedildi.",
        type: "success",
      }),
    );
  };
  ...
  const userContext = useMemo(
    () => ({
      ...
      onDelete: async (id) => {
        if (
          window.confirm("Bu kullanıcıyı silmek istediğinize emin misiniz?")
        ) {
          await dispatch(deleteUser(id));
          dispatch(
            addToast({ message: "Kullanıcı silindi.", type: "success" }),
          );
        }
      },
    }),
  ...
  const bookColDefs = useMemo(
    () => [
      ...
      { field: "isbn", headerName: "ISBN", minWidth: 140 },
      {
        ...
        minWidth: 150,
        valueGetter: (params) =>
          params.data ? getAuthorName(params.data.authorId) : "",
      },
      {
        field: "categoryId",
        headerName: "Kategori",
        minWidth: 130,
        valueGetter: (params) =>
          params.data ? getCategoryName(params.data.categoryId) : "",
      },
      {
        field: "publisherId",
        headerName: "Yayınevi",
        minWidth: 130,
        valueGetter: (params) =>
          params.data ? getPublisherName(params.data.publisherId) : "",
      },
      {
        field: "status",
        headerName: "Durum",
        ...
        cellRenderer: (params) => (
          <span
            className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase ${params.value === "available" ? "bg-success/20 text-success" : "bg-error/20 text-error"}`}
          >
      ...
      },
      {
        headerName: "İşlemler",
        minWidth: 170,
        cellRenderer: BookActionCellRenderer,
        sortable: false,
        filter: false,
      },
    ],
  ...
  const userColDefs = useMemo(
    () => [
      ...
      { field: "name", headerName: "Kullanıcı Adı", minWidth: 200 },
      {
        ...
        minWidth: 140,
        cellRenderer: (params) => (
          <span className="capitalize font-semibold text-accent-gold">
            {params.value}
          </span>
        ),
      },
  ...
  const reservationColDefs = useMemo(
    () => [
      { field: "id", headerName: "ID", width: 80, maxWidth: 100, flex: 0 },
      {
        ...
        minWidth: 180,
        valueGetter: (params) =>
          params.data ? getUserName(params.data.userId) : "",
      },
      {
        field: "bookId",
        headerName: "Kitap",
        minWidth: 200,
        valueGetter: (params) =>
          params.data ? getBookTitle(params.data.bookId) : "",
      },
      { field: "date", headerName: "Tarih", minWidth: 130 },
      {
        ...
        cellRenderer: (params) => (
          <span
            className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase ${params.value === "approved" ? "bg-success/20 text-success" : "bg-accent-gold/20 text-accent-gold"}`}
          >
            {params.value}
          </span>
        ),
      },
      {
        headerName: "İşlemler",
        minWidth: 185,
        cellRenderer: ReservationActionCellRenderer,
        sortable: false,
        filter: false,
      },
    ],
  ...
  const loanColDefs = useMemo(
    () => [
      ...
      { field: "borrowDate", headerName: "Ödünç Tarihi", minWidth: 130 },
      {
        ...
        cellRenderer: (params) => (
          <span
            className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase ${params.value === "overdue" ? "bg-error/20 text-error" : "bg-success/20 text-success"}`}
          >
            {params.value}
          </span>
        ),
      },
      {
        headerName: "İşlemler",
        minWidth: 125,
        cellRenderer: LoanActionCellRenderer,
        sortable: false,
        filter: false,
      },
    ],
  ...
  const eventColDefs = useMemo(
    () => [
      ...
      { field: "location", headerName: "Konum", minWidth: 150 },
      {
        ...
        valueGetter: (params) =>
          params.data
            ? `${params.data.reservedSeats || 0} / ${params.data.capacity || 0}`
            : "",
      },
      {
        headerName: "İşlemler",
        minWidth: 170,
        cellRenderer: EventActionCellRenderer,
        sortable: false,
        filter: false,
      },
    ],
    ...
    <div className="space-y-lg text-left">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-md">
        <div className="space-y-xs">
          ...
          <p className="font-body-md text-on-surface-variant">
            Sistem veritabanını, çalışma alanlarını, ödünç sistemini ve ayarları
            kontrol edin.
          </p>
        </div>
        ...
        <div className="flex gap-sm">
          {activeTab === "books" && (
            <button
              ...
              className="px-md py-2 bg-ember-orange text-white text-xs font-bold rounded-lg hover:opacity-90 transition-all cursor-pointer shadow-glow-accent"
            >
              + Kitap
            </button>
          )}
        ...
        <div className="space-y-lg">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-md">
            ...
            <div className="glass-card p-md rounded-xl border border-outline-variant">
              ...
              <h3 className="text-2xl font-bold mt-1 text-ember-orange">
                {activeLoans.length}
              </h3>
            </div>
            <div className="glass-card p-md rounded-xl border border-outline-variant">
              ...
              <h3 className="text-2xl font-bold mt-1 text-accent-gold">
                {activeRes.length}
              </h3>
            </div>
            <div className="glass-card p-md rounded-xl border border-outline-variant">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
            ...
            <div className="glass-card p-lg rounded-xl border border-outline-variant space-y-md">
              <h3 className="text-base font-bold text-on-surface">
                Canlı Kütüphane Alan Dolulukları
              </h3>
              <div className="space-y-sm">
                <div>
                  ...
                  <div className="w-full bg-surface-container-highest h-2 rounded-full mt-1">
                    <div
                      className="bg-ember-orange h-full rounded-full"
                      style={{ width: `${deskOccupancy}%` }}
                    ></div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="w-full bg-surface-container-highest h-2 rounded-full mt-1">
                    <div
                      className="bg-vivid-purple h-full rounded-full"
              ...
              <div className="flex items-center justify-around py-sm">
                ...
                <div className="space-y-xs text-xs">
                  <div className="flex items-center gap-xs">
                    <span className="w-2.5 h-2.5 bg-vivid-purple rounded-full"></span>{" "}
                    Dijital (PDF/Sesli) %60
                  </div>
                  <div className="flex items-center gap-xs">
                    <span className="w-2.5 h-2.5 bg-ember-orange rounded-full"></span>{" "}
                    Fiziksel %40
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-sm">
          <div className="flex justify-between items-center font-body-md">
            <span className="text-xs text-on-surface-variant font-medium">
              Toplam {books.length} kayıt listeleniyor
            </span>
            <input
              type="text"
      ...
      {activeTab === "users" && (
        <div className="space-y-sm">
          <div className="flex justify-between items-center font-body-md">
            <span className="text-xs text-on-surface-variant font-medium">
              Toplam {users.length} üye listeleniyor
            </span>
            <input
              type="text"
              placeholder="Tabloda hızlı ara..."
      ...
      {activeTab === "reservations" && (
        <div className="space-y-sm">
          <div className="flex justify-between items-center font-body-md">
            <span className="text-xs text-on-surface-variant font-medium">
              Toplam {reservations.length} talep listeleniyor
            </span>
            <input
              type="text"
              placeholder="Tabloda hızlı ara..."
      ...
      {activeTab === "loans" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-lg">
          <div className="glass-card p-lg rounded-xl border border-outline-variant space-y-md">
            <h3 className="text-base font-bold text-on-surface border-l-4 border-ember-orange pl-xs font-body-md">
              Kitap Ödünç Ver
            </h3>
            <form onSubmit={handleIssueLoan} className="space-y-md">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setLoanForm({ ...loanForm, userId: e.target.value })
                  }
              ...
              <button
                type="submit"
                className="w-full py-2 bg-ember-orange text-white text-sm font-bold rounded-lg shadow-md cursor-pointer"
              >
                Kitabı Ödünç Ver
              </button>
            </form>
          ...
          <div className="lg:col-span-2 space-y-sm">
            <div className="flex justify-between items-center font-body-md">
              <span className="text-xs text-on-surface-variant font-medium">
                Toplam {activeBorrowedList.length} aktif kayıt
              </span>
              <input
                type="text"
                placeholder="Tabloda hızlı ara..."
      ...
      {activeTab === "events" && (
        <div className="space-y-sm">
          <div className="flex justify-between items-center font-body-md">
            <span className="text-xs text-on-surface-variant font-medium">
              Toplam {events.length} etkinlik listeleniyor
            </span>
            <input
              type="text"
              placeholder="Tabloda hızlı ara..."
      ...
      {activeTab === "spaces" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-lg font-body-md">
          <div className="space-y-md">
            <h3 className="text-base font-bold text-on-surface border-l-4 border-ember-orange pl-xs">
              Masa Listesi
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-sm">
              {studyDesks.map((desk) => (
                <div
                  key={desk.id}
                  className="bg-surface-container p-md rounded-xl border border-outline-variant flex flex-col justify-between items-center text-center"
                >
                  <span className="font-metadata-mono text-xs text-on-surface-variant">
                    Desk #{desk.id}
                  </span>
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase mt-1 ${desk.status === "available" ? "bg-success/20 text-success" : "bg-error/20 text-error"}`}
                  >
                    {desk.status}
                  </span>
                </div>
          ...
          <div className="space-y-md">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-sm">
              {meetingRooms.map((room) => (
                <div
                  key={room.id}
                  className="bg-surface-container p-md rounded-xl border border-outline-variant flex justify-between items-center"
                >
                  <div>
                  ...
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${room.status === "available" ? "bg-success/20 text-success" : "bg-error/20 text-error"}`}
                  >
                    {room.status}
                  </span>
                </div>
      ...
      {activeTab === "settings" && (
        <div className="glass-card p-lg rounded-xl border border-outline-variant max-w-2xl font-body-md">
          <h3 className="text-base font-bold text-on-surface border-l-4 border-ember-orange pl-xs mb-md">
            Sistem Limit Ayarları
          </h3>
          <form onSubmit={handleSettingsSave} className="space-y-md text-sm">
            <div className="grid grid-cols-2 gap-sm">
              <div>
                <label className="block text-xs font-semibold text-on-surface-variant mb-1 uppercase">
                  Maksimum Ödünç Süresi (Gün)
                </label>
                <input
                  type="number"
                  ...
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      maxLoanDays: Number(e.target.value),
                    })
                  }
                />
              </div>
            ...
            <button
              type="submit"
              className="px-lg py-2.5 bg-ember-orange text-white rounded-lg font-bold text-sm cursor-pointer"
            >
              Ayarları Kaydet
            </button>
          </form>
        ...
        <form onSubmit={handleBookSubmit} className="space-y-md text-sm">
          ...
          <div className="grid grid-cols-3 gap-sm font-body-md">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setBookForm({ ...bookForm, publishYear: e.target.value })
                }
              />
            </div>
            <div>
                  ...
                  onChange={async (e) => {
                    ...
                    if (file) {
                      try {
                        ...
                        dispatch(
                          addToast({
                            message: "Resim sıkıştırılarak yüklendi.",
                            type: "info",
                          }),
                        );
                      } catch {
                  ...
                  onKeyPress={(e) => {
                    if (e.key === "Enter" && e.target.value.trim()) {
                      ...
                      if (!bookForm.keywords.includes(keyword)) {
                        setBookForm({
                          ...bookForm,
                          keywords: [...bookForm.keywords, keyword],
                        });
                      }
                      e.target.value = "";
                    }
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() =>
                          setBookForm({
                            ...bookForm,
                            keywords: bookForm.keywords.filter(
                              (k) => k !== keyword,
                            ),
                          })
                        }
          ...
          <div className="pt-sm flex justify-end gap-sm font-body-md">
            <button
              ...
              className="px-md py-2 border border-outline rounded-lg text-on-surface hover:bg-surface-container-high transition-all cursor-pointer"
            >
              İptal
            </button>
            <button
              type="submit"
              className="px-md py-2 bg-ember-orange text-white rounded-lg font-bold cursor-pointer"
            >
              Kaydet
            </button>
          </div>
        ...
        <form onSubmit={handleSpaceSubmit} className="space-y-md text-sm">
          ...
          <div className="pt-sm flex justify-end gap-sm font-body-md">
            <button
              ...
              className="px-md py-2 border border-outline rounded-lg hover:bg-surface-container-high transition-all cursor-pointer"
            >
              İptal
            </button>
            <button
