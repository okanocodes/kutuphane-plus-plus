...
export const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (registerUser.fulfilled.match(resultAction)) {
      dispatch(addToast({ message: 'Kayıt işlemi başarılı!', type: 'success' }));
      navigate('/dashboard');
    } else {
      dispatch(addToast({ message: resultAction.payload || 'Kayıt başarısız.', type: 'error' }));
    }
  };
  ...
  return (
    <div className="space-y-lg">
      <div className="text-center">
        <h2 className="font-headline-h2 text-2xl font-bold text-on-surface">Yeni Hesap Oluşturun</h2>
        <p className="font-body-md text-sm text-on-surface-variant">Kütüphane sistemine katılmak için bilgilerinizi girin</p>
      </div>
      ...
      <form className="space-y-lg" onSubmit={handleSubmit}>
        ...
        <div className="text-center text-sm font-label-sm">
          <span className="text-on-surface-variant">Zaten hesabınız var mı? </span>
          <Link to="/login" className="font-semibold text-vivid-purple hover:text-ember-orange transition-colors">
            Giriş Yapın
          </Link>
        </div>
      </form>
